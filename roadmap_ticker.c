...
static RoadMapImage gImageOpen, gMiddleImage, gImageClose, gImageDivider;
...
void roadmap_ticker_display() {
   ...
   start = 1;
   start_x = roadmap_canvas_image_width(gImageOpen)+1;
...
void roadmap_ticker_initialize(void){
	...
	roadmap_config_declare_enumeration ("user", &ShowTickerCfg, NULL, "yes", "no", NULL);
  	...
  	gImageOpen = (RoadMapImage) roadmap_res_get(RES_BITMAP, RES_SKIN, TICKER_OPEN_IMAGE);
	if (gImageOpen == NULL){
		roadmap_log (ROADMAP_ERROR, "roadmap_ticker - cannot load %s image ", TICKER_OPEN_IMAGE);
	}

  	gImageClose = (RoadMapImage) roadmap_res_get(RES_BITMAP, RES_SKIN, TICKER_CLOSED_IMAGE);
	if (gImageOpen == NULL){
		roadmap_log (ROADMAP_ERROR, "roadmap_ticker - cannot load %s image ", TICKER_CLOSED_IMAGE);
		return;
	}
