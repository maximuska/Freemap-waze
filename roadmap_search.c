...
static int on_navigate(void *data){
   ...
   if (roadmap_address_show(city_name, street_name, street_number, position, context->context, TRUE)){
...
static int on_show(void *data){
   ...
   char *street_number;
    ...
    if (roadmap_address_show(city_name, street_name, street_number, position, context->context, FALSE)){
      hide_our_dialogs(dec_close);
    }
