        ...
        count = roadmap_plugin_get_closest
		  ...
		  if (!count) {
	        count = roadmap_street_get_closest
	                    (position, scale, layers, layer_count, neighbours, max);
		  }
...
int roadmap_navigate_retrieve_line
    ...
    if (roadmap_navigate_get_neighbours
           (position, scale, accuracy, &closest, 1, type) <= 0) {
...
int roadmap_navigate_retrieve_line_force_name
    ...
    n = roadmap_navigate_get_neighbours
           (position, scale, accuracy, candidate, 5, type);
    if (n <= 0) {
       return -1;
    }
...
void roadmap_navigate_locate (const RoadMapGpsPosition *gps_position) {
    ...
    count = roadmap_navigate_get_neighbours
                (&RoadMapLatestPosition, 0, roadmap_fuzzy_max_distance(),
                 RoadMapNeighbourhood, ROADMAP_NEIGHBOURHOUD - 1, LAYER_ALL_ROADS);

    if (RoadMapConfirmedStreet.valid) {
       /* Always put the current as a candidate - required as the current
        * GPS point may be on a different tile than the current line
        * and thus it may not return as a candidate...
        */
       RoadMapNeighbourhood[count] = RoadMapConfirmedLine;
       count++;
    }
    ...
    for (i = 0, best = roadmap_fuzzy_false(), found = 0, alt_found = 0; i < count; ++i) {
        ...
        if (RoadMapConfirmedStreet.valid && (i < (count - 1)) &&
           roadmap_plugin_same_db_line (&RoadMapNeighbourhood[i].line,
              &RoadMapConfirmedLine.line)) {

           /* The last candidate is always the current confirmed line,
            * so if we found the current line as a regular candidate,
