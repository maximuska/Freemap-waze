...
static void request_tile (int tile_id, int urgent) 
{
	...
	(*tile_status) |= (ROADMAP_TILE_STATUS_FLAG_CALLBACK | ROADMAP_TILE_STATUS_FLAG_ROUTE);
	if (urgent) {
...
static void instrument_segments (int initial) 
{
	...
	if (RoutingContext.route.num_instrumented > prev_done) {
		if (RoutingContext.on_instrumented && !initial) {
			RoutingContext.on_instrumented (RoutingContext.route.num_instrumented);
		}
	}
}
...
static void on_route_complete (void) 
{
	...
	if (RoutingContext.route.num_received == RoutingContext.route.num_segments) {
		...
		if (RoutingContext.on_segments) {
			RoutingContext.on_segments (RoutingContext.route_rc, RoutingContext.result, &RoutingContext.route);
...
{
	roadmap_messagebox("Error", description);
	if (RoutingContext.route_rc == route_succeeded)
		RoutingContext.route_rc = route_server_error;
	
	if (RoutingContext.on_results) {
		RoutingContext.on_results (RoutingContext.route_rc, 0, NULL);
	}	
	else if (RoutingContext.on_segments) {
		RoutingContext.on_segments (RoutingContext.route_rc, NULL, NULL);	
	}
}
   ...
   if (!data) {
	...
	if (geometry->valid_points == geometry->num_points) {
		...
		if (RoutingContext.num_geometries == RoutingContext.num_results &&
			 RoutingContext.on_results != NULL) {
			RoutingContext.on_results (RoutingContext.route_rc, RoutingContext.num_results, RoutingContext.result);
		} 	
	}   	
                             ...
                             NavigateOnRouteInstrumented cb_instrumented)
{
	...
	RoutingContext.on_results = cb_results;
	RoutingContext.on_segments = cb_segments;
	RoutingContext.on_instrumented = cb_instrumented;
	...
	option_values[num_options] = (navigate_cost_avoid_trails () == AVOID_TRAILS_ALL) ? TRUE : FALSE;
	num_options++;
	
	option_types[num_options] = AVOID_LONG_TRAILS;
	option_values[num_options] = (navigate_cost_avoid_trails () == AVOID_TRAILS_LONG) ? TRUE : FALSE;
	num_options++;
