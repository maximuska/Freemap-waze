...
#endif
        roadmap_log( ROADMAP_FATAL, "roadmap_main.cpp::handleException()");
...
/*/[BOOKMARK]:[DISABLED-CODE]:[PAZ] - Auto-sync when application is down - code disabled 7/1/09
         if( do_sync)
         {
            roadmap_log( ROADMAP_DEBUG, "InitInstance(PID: 0x%08X) - Posting 'WM_FREEMAP_SYNC' message to other instance", dwProcessID);
            PostMessage(hWnd, WM_FREEMAP_SYNC, 0, 0);
         }
         else*/
...
#if(defined UNDER_CE && !defined _ROADGPS)
/*/[BOOKMARK]:[DISABLED-CODE]:[PAZ] - Auto-sync when application is down - code disabled 7/1/09
   if (do_sync) {
      ...
      editor_main_set (1);
      roadmap_log( ROADMAP_INFO, "InitInstance(PID: 0x%08X) - Performing sync!", dwProcessID);
      roadmap_main_start_sync ();
#ifdef SSD
      ssd_dialog_wait ();
#endif
...
BOOL OnKeyUp( WORD key)
{
   ...
   {
      if( bEscape)
         return HandleKeyboardKey( ESCAPE_KEY, 0);
      // Else
      return FALSE;
   }
...
static bool cm_is_active = false;
void on_tc_closed(void)
{
   cm_is_active = false;
}

extern "C" void address_tabcontrol_show( RoadMapCallback on_tabcontrol_closed);
