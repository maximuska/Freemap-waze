	...
	rect->miny -= 1;
		...
		if (!header_image)
			header_image = 	roadmap_res_get(
                                    ...
                                    RES_SKIN|RES_LOCK,
                                    "header");
		num_images = (rect->maxx/roadmap_canvas_image_width(header_image))+1;
		if (header_image){
			for (i=0;i<=num_images;i++){
				point.x = i * roadmap_canvas_image_width(header_image);
				point.y = rect->miny;
				roadmap_canvas_draw_image (header_image, &point, 0, IMAGE_NORMAL);
			}
		}
   ...
   if (widget->flags & SSD_CONTAINER_BORDER) {
	 ...
	 RoadMapPosition *position = NULL;
		 ...
		 if (widget->flags & SSD_ROUNDED_CORNERS){
		 	...
		 	if (widget->flags & SSD_HEADER_RED)
		 		header_type = HEADER_RED;
			...
			widget->flags &=  ~SSD_POINTER_MENU;
		 	...
		 	background = "#d2dfef";
		 	if ((header_type != HEADER_GREEN) && (header_type != HEADER_RED))
...
static void add_title (SsdWidget w, int flags) {
  ...
  if (!((w->flags & SSD_DIALOG_FLOAT) && !(w->flags & SSD_DIALOG_TRANSPARENT))){
   ...
   if (!( ((flags & SSD_DIALOG_FLOAT)&& !(flags & SSD_DIALOG_TRANSPARENT)) || (flags & SSD_DIALOG_NO_BACK))){
	   ...
	   ssd_widget_set_click_offsets_ext( btn, 0, -10, 70, 10 );
	   if ( ssd_widget_rtl(NULL) )
	   {
