  ...
  // assert(!s_cbOnDone);
   ...
   if( !s_dialog)
   {
      ...
      box = ssd_container_new ( s_dialogbox_name, NULL, SSD_MAX_SIZE,
               SSD_MIN_SIZE, SSD_END_ROW | SSD_ALIGN_CENTER | SSD_CONTAINER_BORDER | SSD_ALIGN_CENTER);
      ssd_widget_set_color(box, NULL, NULL);
      ssd_widget_set_offset( box, 0, -1 );

      ssd_dialog_add_vspace( box, SSD_KB_DLG_FIELDS_VSPACE, 0 );

      ecnt = ssd_container_new( s_editcnt_name, NULL, entry_width, 40, SSD_CONTAINER_TXT_BOX|SSD_WS_TABSTOP );
...
static void set_label_widget( const char* label, int entry_width, int label_width )
{
	SsdWidget ecnt = ssd_widget_get( s_dialog, s_editcnt_name);
	if ( label == NULL )
	{
	   ssd_widget_hide( ssd_widget_get( s_dialog, s_editlabelcnt_name  ) );
	   ssd_widget_set_size( ecnt, entry_width, 40 );
	}
	else
	{
	   ...
	   ssd_widget_set_size( ecnt, entry_width, 40 );
	   ssd_widget_set_size( label_cnt, label_width, 40 );
