...
ETransactionResult OnHttpResponse( LPResponseBuffer pRI, void* pContext)
{
   ...
   if( pDataSize != NULL ) {
      ...
      // Don't get surprised if content lenght is not present. It's possible to manage without it.
      pRI->iCustomDataSize = INT_MAX;
      roadmap_log( ROADMAP_DEBUG, "RTNet::OnHttpResponse() - Did not find 'Content-Length:' in response (%s), set to %d", pRI->Data, INT_MAX );
   }
