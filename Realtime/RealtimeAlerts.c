...
static RoadMapConfigDescriptor RoadMapConfigMapProblemOptions =
                        ROADMAP_CONFIG_ITEM("General", "Map Problem Options");
...
static  char*  MAP_PROBLEMS_OPTION[] = { MAP_PROBLEM_INCORRECT_TURN,   MAP_PROBLEM_INCORRECT_ADDRESS , MAP_PROBLEM_INCORRECT_ROUTE ,
											MAP_PROBLEM_INCORRECT_MISSING_ROUNDABOUT , MAP_PROBLEM_INCORRECT_GENERAL_ERROR,MAP_PROBLEM_TURN_NOT_ALLOWED,MAP_PROBLEM_INCORRECT_JUNCTION,MAP_PROBLEM_MISSING_BRIDGE_OVERPASS,
												MAP_PROBLEM_WRONG_DRIVING_DIRECTIONS , MAP_PROBLEM_MISSING_EXIT };
...
BOOL RTAlerts_Remove(int iID)
{
    ...
    if (!bFound){
       roadmap_log( ROADMAP_ERROR, "RemoveAlert() - Failed. ID %d not found", iID);
    }
...
static void OnAlertAdd(RTAlert *pAlert)
{
    ...
    {
        gIdleCount = 0;
        roadmap_main_set_periodic(6000, RTAlerts_Timer);
        gTimerActive = TRUE;
    }
...
static void RTAlerts_Popup(void)
{
    ...
    if ((pAlert->iDistance > roadmap_config_get_integer(&RoadMapConfigMaxAlertPopDist))&&
    	(!gIgnoreAlertMaxDist))
    { //// alert is bigger than maximal distance, and should be considered return to closest one at index 0
    	gIterator = 0;
    	pAlert = RTAlerts_Get(gIterator);
    	if (pAlert == NULL)
       		 return;
...
const char *RTAlerts_get_title(int iAlertType, int iAlertSubType){
    ...
    if (iAlertType == RT_ALERT_TYPE_ACCIDENT)
        ...
        return roadmap_lang_get("Police");
...
static void RTAlerts_popup_alert(int alertId, int iCenterAround)
{
   ...
   if (popup){
	...
	bitmap = ssd_bitmap_new("alert_icon", RTAlerts_Get_Icon(pAlert->iID),SSD_WS_TABSTOP);

	ssd_widget_get_size(bitmap, &size, NULL);
    position_con =
      ssd_container_new ("position_container", "", roadmap_canvas_width()-size.width - 32, SSD_MIN_SIZE,0);
	...
	text = ssd_text_new("alert_distance", AlertStr, 14, SSD_END_ROW);
	ssd_widget_add(position_con, text);
	AlertStr[0] = 0;
    ...
    popup = ssd_popup_new("AlertPopUPDlg", RTAlerts_get_title(pAlert->iType, pAlert->iSubType), on_popup_close, SSD_MAX_SIZE, SSD_MIN_SIZE,&position, SSD_HEADER_RED|SSD_POINTER_LOCATION);
...
static void RTAlerts_Comment_PopUp(RTAlertComment *Comment, RTAlert *Alert)
{
	...
	roadmap_config_save(FALSE);
	 ...
	 dialog =  ssd_popup_new("Comment Pop Up", roadmap_lang_get("Response"),NULL,SSD_MAX_SIZE, SSD_MIN_SIZE,NULL, SSD_HEADER_GREEN|SSD_POINTER_COMMENT);
