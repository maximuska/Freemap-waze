...
int prepare_for_upload ()
{
   ...
   res = roadmap_zlib_compress(roadmap_log_path(), roadmap_log_filename(), roadmap_path_debug(), out_filename, COMPRESSION_LEVEL);
   if (res != Z_OK) {
   ...
   for (cursor = files; *cursor != NULL; ++cursor) {
      ...
      res = roadmap_zlib_compress(directory, *cursor, roadmap_path_debug(), out_filename, COMPRESSION_LEVEL);
      if (res != Z_OK) {
         ssd_progress_msg_dialog_hide();
         return 0;
      } else {
...
static void roadmap_confirmed_debug_info_submit(int exit_code, void *context){
   ...
   if (!prepare_for_upload()) {
      roadmap_messagebox("Error", "Error sending files");
      in_process = 0;
      return;
   }
   ...
   in_process = 0;
   roadmap_messagebox("Thank you!!!", "Logs submitted successfully to waze");
}
